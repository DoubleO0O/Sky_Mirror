...
- 随后新增未提交候选：runner `--verify-xdg-disconnect`（仅配合 step，与 admission 验证互斥，仅该模式四批）：第三批保存 admission 动态三元组后，harness 发 release 并等客户端 exit 0 回收，才放行第四批；同一 `State`、owner 释放前验证同一组 client/surface/window 的 tombstone 与 alive=false、surface→client 归属、窗口移出布局、基线窗口及 workspace 归属保持、validation clean。本次实测正向 batches=4、pumps=800 正常停止释放；负向“仍 alive”非零退出且无成功证据。仅证明该有界单客户端场景的 Core 生命周期关闭；不证明 adapter/ledger 完整清理、重连、长期运行、可见窗口、渲染或输入；尚未提交、推送或取得对应 CI。
