...
- 已提交并推送：runner 基础功能（ead1243）及后续步进与双 sync 扩展（75d7b50）；runner 基础功能（默认三批／八秒批间预算、`--step-batches` 每批等 `run N`、socket/lock 独占子目录清理）与 client 双 sync 因果链（sync1 done→run 2→done2→continue→batch3 回包；控制通道容量 1、128 字节过程限长；门控超时、错误编号、输出故障未验证）。默认 main 仍未接入 nested，不得称为 long-running loop 或已发布能力。
- 已提交并推送（bd5dea5）：runner `--verify-xdg-admission`（仅配合 step）与 client `--xdg-admission` 模式：client 真实绑定 globals 并创建 wl_surface/xdg_surface/xdg_toplevel，完成 configure 观察、ack 与无 buffer commit，保持对象至 `release`；runner 在 batch3 后释放前经同一 `State` 基线差分确认动态三元组、归属、存活、workspace 与 validation。门控只保证请求在指定批次开始前已发送；实际正向日志中 Core 窗口已于 batch1 出现，最终在 batch3 后验证。正向双方 exit 0；无客户端时缺新增三元组，非零退出并清理。无 buffer admission 不等于窗口已映射或可见。
- 已提交并推送（64dfd0c）：runner `--verify-xdg-disconnect`（仅配合 step，与 admission 验证互斥，仅该模式四批）：第三批保存 admission 动态三元组后，harness 发 release 并等客户端 exit 0 回收，才放行第四批；同一 `State`、owner 释放前验证同一组 client/surface/window 的 tombstone 与 alive=false、surface→client 归属、窗口移出布局、基线窗口及 workspace 归属保持、validation clean。本次实测正向 batches=4、pumps=800 正常停止释放；负向“仍 alive”非零退出且无成功证据。仅证明该有界单客户端场景的 Core 生命周期关闭；不证明 adapter/ledger 完整清理、重连、长期运行、可见窗口、渲染或输入。
- 已提交并推送（782d9d7，对应 CI 已通过）：runner `--verify-xdg-readmit`（仅配合 step，与另两个 verify 三选一，仅该模式七批，client 未修改）：A 关闭证据后才启动独立客户端 B，批 5–7 完成 B admission；同一 orchestrator、State、socket 保持；动态识别 B，A/B 对应类别 ID 不同、A tombstone 保留、B 存活与归属正确、基线窗口及 workspace 归属保持、validation clean；B 证据后 harness release B 并等 exit 0 回收，再发 `finish` 触发零迭代停止消费——`finish` 非第八批，runner 不直接观察 B 进程退出、未 pump/验证 B Core 关闭。实测正向 A=(1,1,4)、B=(2,2,5)、batches=7、pumps=1400；无 B 与错 finish 均非零。仅证明该有界场景中 A 关闭后仍可接纳 B；不证明完整重连可靠性、adapter/ledger 完整清理、长期运行、可见窗口、渲染、输入或 B Core 关闭。
- 随后新增未提交候选：runner `--verify-xdg-readmit-close`（仅配合 step，与其余 verify 四选一；client 未修改）：沿用 A admission→A 关闭→B admission 的七批流程；B 验证证据后 harness release B 并等待 exit 0 回收，再发送真实第八批 `run 8`（旧 readmit 继续用 `finish`，不执行第八批）；第八批实际 pump 后，在同一 orchestrator、State、socket 内、owner 释放前验证 B 三类记录存在且 dead、surface→client 归属保留、B 窗口退出布局、A 关闭证据保持、workspace 窗口列表恢复启动基线、validation clean，成功后才执行既有零迭代停止消费并释放。实测正向 A=(1,1,4)、B=(2,2,5)、batches=8、pumps=1600、Stopped 且清理成功；B 保持连接的负向因“仍 alive”非零且无第二次断连成功行。本项仅证明同一有界 session 内两轮客户端 Core 生命周期；不证明 adapter/ledger 完整清理、广泛重连可靠性、长期运行、可见窗口、渲染、输入；尚未提交、推送或取得对应 CI。
