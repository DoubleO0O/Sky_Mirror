# Phase 52Q Controlled xdg_surface Creation Proof

## 1. Baseline and Route

- Base: `main@50ca3ee`, including accepted Phase 52P-B commit `31f1f14`.
- Local `main` and `origin/main` matched before the worktree was created.
- The latest main CI runs were green.
- codebase-memory-mcp was checked with project name `Users-double-sky_mirror`;
  only the main project index was used.
- This phase selects **Route B: controlled xdg_surface creation proof**.

No Cargo, core, backend, main, CI, xdg_toplevel lifecycle, ledger, renderer,
input, DRM, GBM, or libinput change is part of this phase.
...
Phase 52Q combines the existing controlled wl_surface and xdg_wm_base paths in
one client-side proof. Both xdg-shell and wl_compositor owners must be
initialized before endpoint creation; missing owners produce structured blockers
...
`get_xdg_surface` creates only an xdg role object for an existing wl_surface.
It is not equivalent to `get_toplevel`, does not allocate a core window, and
does not prove renderability.
...
Therefore:
    ...
    xdg_surface created != xdg_toplevel created
    xdg_surface created != new_toplevel callback observed
    xdg_surface proof != window admission
    bounded dispatch != real compositor runtime
    adapter surface identity != core SurfaceId registration

## 6. Preserved Boundaries

- No `get_toplevel` request.
- No `XdgToplevel` client object in the proof.
...
The next phase may separately prove controlled `xdg_toplevel` creation. It must
still stop before ledger/core admission, render, input, or real daily-use
runtime unless those are authorized as independent phases.
