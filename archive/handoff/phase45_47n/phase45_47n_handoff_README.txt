Sky Mirror Phase 45/46/47M/47N Arch/Linux 验证交接说明
========================================================
...
3. Phase 47M：纯 Rust surface 生命周期模型、注册表、结构化错误和测试。
4. Phase 47N：surface trace runner、执行报告、mock adapter 和场景测试。

Phase 47M/47N 只建立后端中立的纯数据生命周期与 trace 测试边界，不等于真实
...
git apply --check phase45_47n_handoff.patch
git apply phase45_47n_handoff.patch
...
4. runtime_facade.rs、surface_lifecycle.rs、surface_trace.rs 完整应用。
5. Linux Display/socket 测试真实执行。
6. Trace runner 仍只通过 BackendSurfaceRegistry::apply_event 推进状态。
7. core/backend 不依赖 runtime facade、surface lifecycle 或 surface trace 类型。
8. supports_surface_lifecycle_boundary 为 true。
9. supports_surface_trace_harness 为 true。
10. supports_real_wayland_surfaces 为 false。
11. supports_gpu_rendering 为 false。
12. default = [] 不变。
...
不得削弱断言，也不得借验证修复提前实现真实 surface adapter。

七、验证成功后的下一步

只有 Arch/Linux 上 smithay-linux 的编译和测试全部通过，Phase 47 阻塞才可以
解除。成功后才允许进入真实 Smithay adapter 阶段，把平台回调转换为既有
BackendSurfaceLifecycleEvent，并继续遵守 BackendEvent、BackendDriverRunner、
CoreRuntimeBridge 和 CoreCommand 的状态入口边界。
