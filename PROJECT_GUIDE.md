...
readmit 回归轮的新鲜结果：回归矩阵（/tmp/sm-rmreg-dw7I4S/）default 353 passed、smithay-probe 635 passed、smithay-linux 913 passed、0 failed、1 ignored（该 ignored 项仍是 Winit 必须在进程主线程创建的 unit test；controlled_winit、bounded_session_runner、session_probe_client 三个辅助 test binary 均为 0 tests，0 tests 不是功能验收；watchdog child 复跑输出不重复计数）。定向进程验收（/tmp/sm-rm-yJgPZ8/）：readmit 正向实测 A=(1,1,4)、B=(2,2,5)、batches=7、pumps=1400，正常停止并清理；无 B 因缺少新增 alive client 非零；错误 finish 命令非零；旧 admission／disconnect／双 sync／默认无参定向验收均通过。ID 与计数是本次实测，不是硬编码契约。两处日志均为临时目录证据，不保证永久保留。all-features 本轮未重跑，不得引用旧 all-features 数字作为本轮结论（此前某轮 all-features 908 tests／907 passed／1 ignored、clippy 退出码 0 附 113 条既有非致命 warning 仍为历史记录）。readmit 扩展尚未提交、推送或取得对应 CI。
