...
- 独立非测试 `src/bin/sky_mirror_bounded_session_runner.rs`（基础功能已提交；XDG admission（bd5dea5）、断连验证（64dfd0c）、readmit 七批＋finish（782d9d7）均已提交、推送并通过对应 CI；readmit-close 扩展仍未提交）复用该 seam。默认模式保持原有行为：最多三个正常批次，或批间观察到八秒预算耗尽后经 stop handle 停止；八秒只是批间检查预算，不是能中断当前批次的硬超时。`--step-batches` 受控模式每批前输出 `batch ready: n=N` 并有界等待精确命令 `run N`（单例 reader 线程＋固定容量同步通道，每门十秒等待预算，读取过程按 128 字节含行终止符限长），停止消费批次不设门；八秒预算不用于 step 模式。`--verify-xdg-admission`（仅配合 step）在 batch3 完成后、owner 释放前，经同一 `State` 的启动前基线差分确认动态 client/surface/window 身份、归属、存活、workspace 与 validation，成功才打印 Core 证据行。`--verify-xdg-disconnect`（仅配合 step，与 admission 验证互斥）仅该模式增加第四个正常批次：第三批后保存 admission 动态三元组，经 harness 发 release、等待客户端 exit 0 并回收后才放行第四批，随后在同一 `State`、owner 释放前验证同一组 client/surface/window 的 tombstone 与 alive=false、surface→client 归属、窗口移出 slot/stack、基线窗口与 workspace 归属保持、validation clean；本次实测正向 batches=4、pumps=800 并正常停止释放，保持客户端连接的负向场景因“仍 alive”非零退出（均为实测，非所有运行保证）。新增 `--verify-xdg-readmit`（仅配合 step，与另两个 verify 三选一）：仅该模式七个正常批次，旧模式批数与预算不变，client 未修改；A admission→release A 并由 harness 等待 exit 0 回收→第四批验证 A 关闭→才启动独立客户端 B→第五至七批完成 B admission；同一 orchestrator、State、socket 保持，动态识别 B 并确认 A/B 对应类别 ID 不同、A tombstone 保留、B 存活与归属正确、基线窗口及 workspace 归属保持、validation clean；B 证据后 harness release B 并等待 exit 0 回收，再发 `finish`，runner 零迭代停止消费与释放——`finish` 不算第八个正常批次，runner 只收到 harness 确认，不直接观察 B 进程退出，也未 pump 或验证 B 的 Core 关闭。新增 `--verify-xdg-readmit-close`（仅配合 step，与其余 verify 四选一；仍未提交）：沿用 A admission→A 关闭→B admission 的七批流程；B 验证证据后 harness release B 并等待 exit 0 回收，再发送真实第八批命令 `run 8`（旧 readmit 继续用 `finish`，不执行第八批）；第八批实际 pump 后，在同一 orchestrator、State、socket 内、owner 释放前验证 B 三类记录存在且 dead、surface→client 归属保留、B 窗口退出布局、A 关闭证据保持、workspace 窗口列表恢复启动基线、validation clean，成功后才执行既有零迭代停止消费并释放。socket/lock 隔离在 XDG 下的独占私有 0700 子目录内，不删除外层 XDG。
- 独立非测试 `src/bin/sky_mirror_session_probe_client.rs`（双 sync 与 xdg 模式均已提交，bd5dea5）：同一连接完成 registry 请求及两次带独立标识的 sync，不创建 surface/toplevel。已验证因果链：收到 sync1 done 后才发 run 2，收到 done2 后才发 continue，第二次 sync 在 batch3 得到回包；仅证明同一外部客户端连接跨越完整服务端批次后仍能响应新 sync。新增显式 `--xdg-admission` 模式：真实绑定所需 globals，创建 wl_surface/xdg_surface/xdg_toplevel，处理 configure、ack 及无 buffer commit，保持对象至 `release` 门控。门控只保证请求在指定批次开始前已发送，不保证其首次处理发生在指定批次；实际正向日志中 Core 窗口已于 batch1 出现，最终在 batch3 后验证。两控制入口均用固定容量同步通道（容量为 1），输入上限 128 字节含行终止符且在读取过程中限长；已验证 EOF、超长无换行输入、128 字节内容加换行的拒绝与清理。reader 线程的阻塞读取不可由 `recv_timeout` 取消，主流程超时后收尾并退出进程，线程随进程退出回收（不宣称已 join）。门控超时、错误编号、输出故障等负向场景仍未验证。默认 main 仍未接入 nested。
...
- 可持续运行且具有明确停止机制的 nested session 驱动、long-running loop（`long_running_loop_available` 仍为 false）、持续渲染与输入接入。
...
feature-gated Linux nested 路径：
    ...
    NestedRuntimeOrchestrator
      -> run_next_batch（crate 内重复有界批次 seam；由独立 bounded_session_runner 调用，另有 session_probe_client 独立验收；断连验证扩展未提交）
      -> NestedRuntimeLoop
...
readmit-close 回归轮的新鲜结果：回归矩阵（/tmp/sm-rrc-PJcOrI/）default 353 passed、smithay-probe 635 passed、smithay-linux 913 passed、0 failed、1 ignored（该 ignored 项仍是 Winit 必须在进程主线程创建的 unit test；controlled_winit、bounded_session_runner、session_probe_client 三个辅助 test binary 均为 0 tests，0 tests 不是功能验收；watchdog child 复跑输出不重复计数）。定向进程验收（/tmp/sm-rc-gaqBne/）：readmit-close 正向实测 A=(1,1,4)、B=(2,2,5)、batches=8、pumps=1600，Stopped 且清理成功；B 保持连接的负向因“仍 alive”非零，无第二次断连成功行（首次正向曾因 harness 解包错误失败，修正 harness 后通过，该失败非生产行为失败）；旧 readmit（finish）、disconnect、admission、双 sync、默认无参定向验收均通过。ID 与计数是本次实测，不是硬编码成功条件。两处日志均为临时目录证据，不保证永久保留。all-features 本轮未重跑，不得引用旧 all-features 数字作为本轮结论（此前某轮 all-features 908 tests／907 passed／1 ignored、clippy 退出码 0 附 113 条既有非致命 warning 仍为历史记录）。readmit-close 扩展尚未提交、推送或取得对应 CI。
...
1. 用户路径仍未汇合：默认 main 尚未接入 production nested orchestrator；已有独立非测试 bounded_session_runner 候选驱动有限批次，但不等于日常桌面入口或 long-running compositor。
